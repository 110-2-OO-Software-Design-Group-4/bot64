...
def b_search_word(l, r, substring, sen_words, substrlen):#二分搜敏感字詞，預設敏感詞已經經過排序
    if l <= r:
        mid = (l+r)//2
        token = substring[:min(substrlen,len(sen_words[mid]))]
        token = remove_punctuation(token)
        substring = remove_punctuation(substring)
        ...
        if sen_words[mid] > substring:
            return b_search_word(l, mid-1, substring, sen_words, substrlen)
        elif sen_words[mid] < substring:
            return b_search_word(mid+1, r, substring, sen_words, substrlen)
    else:
        return 0.
...
async def scanner(message: Message) -> Literal[MessageFlag.Safe, MessageFlag.Suspicious, MessageFlag.Malicious]:    
    ...
    for i in range(N_char):
        ...
        judge1 = b_search_word(0, Num_word - 1, substring1, sen_words, substrlen1)#文字訊息首位，或前一位為空白則以其當字首切割子字串，用二分搜判斷敏感詞
        if(judge1 == 0.):
            judge1 += b_search_word(0, Num_word - 1, substring2, sen_words, substrlen2)#敏感字串三種字串切割類型都要檢查
        if(judge1 == 0.):
            judge1 += b_search_word(0, Num_word - 1, substring3, sen_words, substrlen3)
            
        judge += judge1
        judge += b_search_link(0, Num_link - 1, substring2, sen_links, substrlen2)#以相同訊息判斷釣魚連結
